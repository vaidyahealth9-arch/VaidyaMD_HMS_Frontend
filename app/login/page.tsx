...
export default function LoginPage() {
  ...
  const getRoleDestination = (role?: string) => {
    ...
    if (r === 'receptionist' || r === 'accounts') return '/appointments';
    if (r === 'pharma' || r === 'pharmacist') return '/pharmacy?tab=pos';
    if (r === 'embryologist' || r === 'andrologist') return '/ivf-lab';
    if (r === 'counsellor' || r === 'counselor') return '/counseling';
    if (r === 'doctor') return '/appointments?status=waiting';
    return '/dashboard';
  };
        ...
        <div className="w-full max-w-md">
          ...
          <form onSubmit={handleLogin} className="space-y-5">
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full bg-[rgb(var(--clr-primary))] text-white font-semibold py-2.5 rounded-md hover:bg-[rgb(var(--clr-primary)/0.9)] transition-colors shadow-sm disabled:opacity-60 disabled:cursor-not-allowed flex items-center justify-center gap-2 text-sm cursor-pointer"
            >
              {isLoading ? (
                <>
