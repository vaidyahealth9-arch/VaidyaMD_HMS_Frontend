          ...
          <table className="w-full text-left text-xs border-collapse">
            ...
            <thead className="sticky top-0 bg-slate-100 z-10 shadow-2xs">
              <tr className="border-b border-slate-200">
                <th className="p-3.5 min-w-[220px] bg-slate-100 text-slate-800 font-bold text-xs sticky left-0 z-20 shadow-r">
                  Medication / Parameter
                </th>
                {days.map((d) => (
                  <th
                    key={d.day_number}
                    className={`p-2.5 min-w-[90px] max-w-[110px] text-center border-l border-slate-200 ${
                      d.milestone
                        ? 'bg-indigo-50/80 text-indigo-950 font-bold'
                        : 'text-slate-700 font-bold'
                    }`}
                  >
                    <div className="text-[11px] uppercase tracking-wide">Day {d.day_number}</div>
                    <div className="text-[10px] font-medium text-slate-500">{d.display_date}</div>
                    <div className="text-[9px] text-slate-400 uppercase">{d.day_of_week}</div>
                    {d.milestone && (
                      <span className="inline-block mt-1 text-[8px] font-bold uppercase tracking-tight px-1 py-0.5 rounded bg-indigo-200/70 text-indigo-900 truncate max-w-[90px]">
                        {d.milestone}
                      </span>
                    )}
                  </th>
            ...
            <tbody className="divide-y divide-slate-100">
              ...
              <tr className="bg-slate-50/80">
                <td
                  ...
                  className="py-2 px-3 text-[11px] font-bold uppercase tracking-wider text-indigo-900 bg-indigo-50/40"
                >
                  <span className="flex items-center gap-1.5"><Pill className="w-3.5 h-3.5 text-slate-600" /> Stimulation Medications &amp; Injections</span>
                </td>
              </tr>

              {drugList.map((drugName) => (
                <tr key={drugName} className="hover:bg-slate-50/40 transition-colors">
                  {/* Sticky Drug Name Header */}
                  <td className="p-3 font-bold text-slate-800 bg-white sticky left-0 z-10 shadow-r flex items-center justify-between group">
                    <span className="text-xs truncate max-w-[180px]" title={drugName}>
                      {drugName}
                    </span>
                    {!readonly && (
                      <button
                        ...
                        className="opacity-0 group-hover:opacity-100 text-slate-300 hover:text-rose-600 transition-opacity p-0.5"
                        title="Remove row"
                      >
                  ...
                  {days.map((day) => {
                    ...
                    return (
                      <td
                        key={day.day_number}
                        className="p-1.5 border-l border-slate-100 text-center relative group/cell"
                      >
                        <input
                          type="text"
                          value={doseVal}
                          ...
                          className={`w-full text-center text-xs py-1 px-1 rounded-lg transition-all ${
                            doseVal
                              ? 'font-bold bg-indigo-50/80 text-indigo-900 border border-indigo-200'
                              : 'text-slate-400 bg-transparent hover:bg-slate-50 border border-transparent focus:border-indigo-300'
                          }`}
                        />
                        ...
                        {!readonly && doseVal && day.day_number < totalDays && (
                          <button
                            ...
                            className="hidden group-hover/cell:flex items-center justify-center absolute -right-2 top-1/2 -translate-y-1/2 w-4 h-4 rounded-full bg-indigo-600 text-white z-20 shadow-xs hover:scale-110 transition-transform"
              ...
              {/* SECTION 2: FOLLICULOMETRY */}
              <tr className="bg-slate-50/80">
                <td
                  colSpan={days.length + 1}
                  className="py-2 px-3 text-[11px] font-bold uppercase tracking-wider text-rose-900 bg-rose-50/40"
                >
                  <span className="flex items-center gap-1.5"><Microscope className="w-3.5 h-3.5 text-rose-600" /> Serial Ultrasound Folliculometry</span>
                </td>
              </tr>

              {/* Right Ovary Follicles */}
              <tr className="hover:bg-slate-50/40">
                <td className="p-3 font-bold text-slate-700 bg-white sticky left-0 z-10 shadow-r">
                  <span className="text-xs">Right Ovary Follicles (mm)</span>
                  <span className="block text-[10px] text-slate-400 font-normal">e.g. 18, 16, 14, 12</span>
                </td>
                {days.map((day) => (
                  <td key={day.day_number} className="p-1.5 border-l border-slate-100 text-center">
                    <input
                      type="text"
                      placeholder="—"
                      ...
                      className={`w-full text-center text-xs py-1 px-1 rounded-lg ${
                        day.right_follicles
              ...
              <tr className="hover:bg-slate-50/40">
                <td className="p-3 font-bold text-slate-700 bg-white sticky left-0 z-10 shadow-r">
                  <span className="text-xs">Left Ovary Follicles (mm)</span>
                  <span className="block text-[10px] text-slate-400 font-normal">e.g. 19, 17, 15, 11</span>
                </td>
                {days.map((day) => (
                  <td key={day.day_number} className="p-1.5 border-l border-slate-100 text-center">
                    <input
              ...
              <tr className="hover:bg-slate-50/40">
                <td className="p-3 font-bold text-slate-700 bg-white sticky left-0 z-10 shadow-r">
                  <span className="text-xs">Endometrial Thickness (mm)</span>
                  <span className="block text-[10px] text-slate-400 font-normal">e.g. 8.4 (Trilaminar)</span>
                </td>
                {days.map((day) => (
                  <td key={day.day_number} className="p-1.5 border-l border-slate-100 text-center">
                    <input
              ...
              {/* SECTION 3: HORMONE SERUM LEVELS */}
              <tr className="bg-slate-50/80">
                <td
                  colSpan={days.length + 1}
                  className="py-2 px-3 text-[11px] font-bold uppercase tracking-wider text-amber-900 bg-amber-50/40"
                >
                  <span className="flex items-center gap-1.5"><Activity className="w-3.5 h-3.5 text-amber-600" /> Endocrine Serum Levels</span>
                </td>
              </tr>
              ...
              <tr className="hover:bg-slate-50/40">
                <td className="p-3 font-bold text-slate-700 bg-white sticky left-0 z-10 shadow-r">
                  <span className="text-xs">Serum E2 (pg/mL)</span>
                </td>
                {days.map((day) => (
                  <td key={day.day_number} className="p-1.5 border-l border-slate-100 text-center">
                    <input
              ...
              <tr className="hover:bg-slate-50/40">
                <td className="p-3 font-bold text-slate-700 bg-white sticky left-0 z-10 shadow-r">
                  <span className="text-xs">Serum P4 (ng/mL)</span>
                </td>
                {days.map((day) => (
                  <td key={day.day_number} className="p-1.5 border-l border-slate-100 text-center">
                    <input
